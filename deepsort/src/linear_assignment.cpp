...
/**
 * @param distance_metric       tracker
 * @param distance_metric_func  特征匹配函数
 * @param max_distance          余弦距离
 * @param cascade_depth         最大年龄
 * @param tracks                轨迹指针
 * @param detections            所有检测
 * @param track_indices         确认状态
 * @param detection_indices
 */
TRACHER_MATCHD
linear_assignment::matching_cascade(
        MultiTracker *distance_metric,
        MultiTracker::GATED_METRIC_FUNC distance_metric_func,
        float max_distance,
        int cascade_depth,
 ...
 */
TRACHER_MATCHD
linear_assignment::min_cost_matching(MultiTracker *distance_metric,
        MultiTracker::GATED_METRIC_FUNC distance_metric_func,
        float max_distance,
        std::vector<Track> &tracks,
        const DETECTIONS &detections,
