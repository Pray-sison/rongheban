...
/**
 * 用于计算两个目标检测框之间的距离，以确定它们是否属于同一个目标
 * @param metric                余弦模式 还有欧式距离模式
 * @param matching_threshold    最大余弦距离
 * @param budget                最大预留轨迹
 */
...
/**
 * @param confirmed状态的track (id, features)
 * @param confirmed状态的track id
 */
void
NearNeighborDisMetric::partial_fit(
        std::vector<TRACKER_DATA> &tid_feats,
        std::vector<int> &active_targets) {
    // 遍历所有的confirmed状态的track(id, features)
